...
const sb = {
  ...
  async upload(file) {
    ...
    const r=await fetch(`${SB_URL}/storage/v1/object/closet-images/${path}`,{method:'POST',headers:{'apikey':SB_KEY,'Authorization':`Bearer ${SB_KEY}`,'Content-Type':file.type},body:file});
    if(!r.ok)return null;
...
const toOR   = o => ({id:o.id,name:o.name,occasion:o.occasion||null,notes:o.notes||null,item_ids:o.itemIds||[],item_positions:o.positions||null,wear_count:o.wearCount||0,last_worn:o.lastWorn??null});
const fromOR = r => ({id:r.id,name:r.name,occasion:r.occasion,notes:r.notes,itemIds:r.item_ids||[],positions:r.item_positions||null,wearCount:r.wear_count||0,lastWorn:r.last_worn});
...
const OUTFIT_SLOTS = ['Top','Bottom','Shoes','Bag','Outer'];
...
const SIZE_GUIDES = [
  ...
  {store:'Lululemon',cat:'Activewear',country:'AU',sizes:[{l:'2',au:'4',bust:'—',waist:'58–61',hip:'83–86'},{l:'4',au:'6',bust:'—',waist:'61–64',hip:'86–89'},{l:'6',au:'8',bust:'—',waist:'64–67',hip:'89–92'},{l:'8',au:'10',bust:'—',waist:'67–70',hip:'92–95'},{l:'10',au:'12',bust:'—',waist:'70–73',hip:'95–98'},{l:'12',au:'14',bust:'—',waist:'73–76',hip:'98–101'}]},
...
const CSS = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;}
  ...
  .icard{border-radius:14px;overflow:hidden;background:#fff;border:1.5px solid var(--border);cursor:pointer;transition:transform .15s,box-shadow .15s;position:relative;}
  .icard:hover{box-shadow:0 4px 16px rgba(0,0,0,.1);}
  .icard:active{transform:scale(0.97);}
  .icard.incomplete{border-style:dashed;}
  .iphoto{width:100%;aspect-ratio:3/4;background:var(--cream);display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden;}
  ...
  .iname{font-size:11px;font-weight:500;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  .imeta{font-size:9.5px;color:var(--muted);margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  .isize{font-size:9.5px;font-weight:500;color:var(--accent);margin-top:1px;}
  .wcard{background:#fff;border-radius:14px;border:1.5px solid var(--border);cursor:pointer;transition:transform .15s,box-shadow .15s;overflow:hidden;}
  ...
  .wimg{width:100%;aspect-ratio:4/3;background:var(--cream);display:flex;align-items:center;justify-content:center;overflow:hidden;}
  .wimg img{width:100%;height:100%;object-fit:cover;}
  .simflag{font-size:9px;background:var(--accent-bg);color:var(--accent);border-radius:5px;padding:2px 6px;margin-top:5px;display:inline-block;}
  ...
  .builder-stage.freeform{cursor:default;}
  .structured-layout{display:flex;flex-direction:column;gap:8px;}
  .slot-row{display:flex;align-items:center;gap:10px;}
  .slot-label{font-size:10px;letter-spacing:.8px;text-transform:uppercase;color:var(--muted);width:52px;flex-shrink:0;}
  ...
  .slot-thumb{width:44px;height:52px;border-radius:6px;overflow:hidden;flex-shrink:0;background:var(--cream);display:flex;align-items:center;justify-content:center;font-size:20px;}
  ...
  .picker-thumb{width:100%;aspect-ratio:3/4;background:var(--cream);display:flex;align-items:center;justify-content:center;overflow:hidden;}
...
function StarDisplay({rating,size=12,onClick}){
  return <div style={{display:'flex',gap:2}}>{[1,2,3,4,5].map(i=><span key={i} onClick={e=>{e.stopPropagation();onClick&&onClick(i);}} style={{fontSize:size,cursor:onClick?'pointer':'default',color:i<=rating?'#C9A050':'#DDD6CA',lineHeight:1}}>★</span>)}</div>;
}
function Toggle({on,onToggle}){
  return <div onClick={e=>{e.stopPropagation();onToggle();}} style={{width:44,height:26,background:on?'#5A8A60':'#E0D9CF',borderRadius:13,position:'relative',cursor:'pointer',transition:'background .2s',flexShrink:0}}><div style={{position:'absolute',top:3,left:3,width:20,height:20,background:'#fff',borderRadius:'50%',transition:'transform .2s',boxShadow:'0 1px 3px rgba(0,0,0,.2)',transform:on?'translateX(18px)':'none'}}/></div>;
}
function PhotoZone({photoUrl,onFile,style={},uploading=false}){
  const ref=useRef();
  return <div className="pz" style={style} onClick={()=>ref.current?.click()}>
    {photoUrl&&<img src={photoUrl} alt=""/>}
    {!photoUrl&&!uploading&&<><span style={{fontSize:26,zIndex:1}}>📷</span><span style={{fontSize:11,color:'var(--muted)',zIndex:1,textAlign:'center',lineHeight:1.4}}>Tap to add photo{'\n'}camera or library</span></>}
    {uploading&&<div style={{zIndex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:6}}><div className="spinner" style={{borderTopColor:'var(--accent)',borderColor:'var(--border)'}}/><span style={{fontSize:11,color:'var(--muted)'}}>Uploading…</span></div>}
    <input ref={ref} type="file" accept="image/*" onChange={e=>{const f=e.target.files?.[0];if(f)onFile(f);}}/>
  </div>;
}
function OccChips({selected,onChange}){
  return <div style={{display:'flex',flexWrap:'wrap',gap:5,marginTop:4}}>{OCCASIONS.map(o=>{const c=OCC_C[o]||{bg:'#eee',b:'#aaa',t:'#333'};const sel=selected.includes(o);return <div key={o} onClick={()=>onChange(sel?selected.filter(x=>x!==o):[...selected,o])} style={{padding:'4px 10px',borderRadius:20,fontSize:11,border:`1.5px solid ${c.b}`,cursor:'pointer',background:sel?c.b:c.bg,color:sel?'#fff':c.t,fontFamily:"'Jost',sans-serif"}}>{o}</div>})}</div>;
}
function StorePicker({value,onChange,stores,onAddStore}){
  const [open,setOpen]=useState(false);const [q,setQ]=useState(value||'');const ref=useRef();
  useEffect(()=>setQ(value||''),[value]);
  const filtered=stores.filter(s=>s.name.toLowerCase().includes(q.toLowerCase()));
  const exact=stores.some(s=>s.name.toLowerCase()===q.toLowerCase());
  ...
  return <div className="store-picker" ref={ref}>
    <input className="f-inp" value={q} placeholder="Search or type store name…" onChange={e=>{setQ(e.target.value);onChange(e.target.value);setOpen(true);}} onFocus={()=>setOpen(true)}/>
    {open&&<div className="store-options">
      {filtered.slice(0,8).map(s=><div key={s.id} className="store-opt" onClick={()=>sel(s.name)}>{s.name}</div>)}
      {q&&!exact&&<div className="store-opt add-new" onClick={()=>{onAddStore(q);sel(q);}}>+ Add "{q}" to stores</div>}
    </div>}
  </div>;
}
...
// ── Photo upload helper ───────────────────────────────────────────────────────
async function handlePhotoFile(file,cb,setUploading){
  setUploading(true);
  const url=await sb.upload(file);
  setUploading(false);
  cb(url||URL.createObjectURL(file));
}
...
function WardrobeFields({d,up,stores,onAddStore}){
  ...
  return <>
    <div style={{display:'flex',gap:12,marginBottom:14}}>
      <PhotoZone photoUrl={d.photoUrl} uploading={pu} onFile={f=>handlePhotoFile(f,u=>up({photoUrl:u}),setPU)} style={{flex:'0 0 110px',height:146}}/>
    ...
    <FRow><FGrp label="Size" style={{flex:1}}><FInp value={d.size} placeholder="S / 10 / 27" onChange={e=>up({size:e.target.value,sizeLabel:e.target.value})}/></FGrp><FGrp label="Date bought" style={{flex:1}}><FInp value={d.dateBought} placeholder="2024-03" onChange={e=>up({dateBought:e.target.value})}/></FGrp></FRow>
    <FGrp label="Store"><StorePicker value={d.store||''} onChange={v=>up({store:v})} stores={stores} onAddStore={onAddStore}/></FGrp>
...
function AddWardrobeSheet({onSave,onClose,stores,onAddStore}){
  const [d,setD]=useState({name:'',category:'Tops',subcategory:'',brand:'',store:'',color:'',size:'',sizeLabel:'',photoUrl:null,occasions:[],dateBought:'',notes:''});
  const [saving,setSaving]=useState(false);
  const up=v=>setD(p=>({...p,...v}));
  async function save(photoOnly=false){setSaving(true);await onSave({...d,id:uid(),complete:!photoOnly&&!!(d.name&&d.category),sizeLabel:d.sizeLabel||d.size,name:d.name||'New item',lastWorn:null,wearCount:0});setSaving(false);}
...
function EditWardrobeSheet({item,onSave,onCancel,stores,onAddStore}){
  ...
  return <Sheet title="Edit item" onClose={onCancel} actions={<><BtnO onClick={onCancel}>Cancel</BtnO><BtnF onClick={async()=>{setSaving(true);await onSave(f);setSaving(false);}} loading={saving} color="#B07848">Save changes</BtnF></>}>
    ...
    <FGrp label="Total wears" style={{marginTop:4}}><FInp value={f.wearCount} type="number" onChange={e=>up({wearCount:parseInt(e.target.value)||0})}/></FGrp>
  </Sheet>;
}
function WardrobeDetailSheet({item,onClose,onEdit,onMarkWorn,onDelete}){
  return <Sheet title={item.name||'Untitled'} onClose={onClose}>
    <div style={{width:'100%',aspectRatio:'3/4',maxHeight:240,background:'var(--cream)',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',borderRadius:14,marginBottom:14}}>
      {item.photoUrl?<img src={item.photoUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:52}}>{CAT_EMOJI[item.category]||'👗'}</span>}
    </div>
    ...
    <DetailRow label="Category" value={item.category}/><DetailRow label="Type" value={item.subcategory}/><DetailRow label="Brand" value={item.brand}/><DetailRow label="Store" value={item.store}/><DetailRow label="Colour" value={item.color}/><DetailRow label="Size" value={item.sizeLabel||item.size}/><DetailRow label="Date bought" value={item.dateBought}/><DetailRow label="Last worn" value={daysLabel(item.lastWorn)}/><DetailRow label="Total wears" value={item.wearCount||0}/><DetailRow label="Occasions" value={(item.occasions||[]).join(', ')||null}/><DetailRow label="Notes" value={item.notes}/>
    <div style={{display:'flex',gap:8,marginTop:14}}>
      <button onClick={onMarkWorn} style={{flex:1,padding:12,background:'var(--green)',color:'#fff',border:'none',borderRadius:12,fontSize:13,cursor:'pointer',fontFamily:"'Jost',sans-serif"}}>✓ Worn today</button>
...
function WishFields({d,up,stores,onAddStore}){
  const [pu,setPU]=useState(false);const [urlIn,setUrlIn]=useState(d.url||'');const [fetching,setFetching]=useState(false);
  const subcats=CATEGORY_MAP[d.category]||[];
  ...
  return <>
    <FGrp label="Paste a link (optional)">
      ...
      <div style={{fontSize:10,color:'var(--muted)',marginTop:3}}>Extracts the store name — fill in the rest manually for now</div>
    </FGrp>
    {d.url&&<div style={{display:'flex',alignItems:'center',gap:6,padding:'8px 12px',background:'var(--cream)',borderRadius:10,marginBottom:10,border:'1px solid var(--border)'}}><span>🔗</span><span style={{fontSize:11,color:'var(--accent)',flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.url}</span></div>}
    <div style={{display:'flex',gap:12,marginBottom:14}}>
      <PhotoZone photoUrl={d.photoUrl} uploading={pu} onFile={f=>handlePhotoFile(f,u=>up({photoUrl:u}),setPU)} style={{flex:'0 0 100px',height:133}}/>
      <div style={{flex:1,display:'flex',flexDirection:'column',gap:8}}>
...
function AddWishSheet({onSave,onClose,stores,onAddStore}){
  const [d,setD]=useState({name:'',store:'',brand:'',url:'',price:'',origPrice:'',onSale:false,rating:3,photoUrl:null,notes:'',category:'Tops',subcategory:'',color:''});
  const [saving,setSaving]=useState(false);const up=v=>setD(p=>({...p,...v}));
  return <Sheet title="Add to wishlist" onClose={onClose} actions={<><BtnO onClick={onClose}>Cancel</BtnO><BtnF onClick={async()=>{setSaving(true);await onSave({...d,id:uid(),addedDate:today()});setSaving(false);}} disabled={!d.name} loading={saving} color="#B07848">Add to wishlist</BtnF></>}>
...
function WishDetailSheet({item,similar,onClose,onEdit,onDelete,onRate,onMoveToWardrobe}){
  return <Sheet title={item.name} onClose={onClose}>
    <div style={{width:'100%',aspectRatio:'4/3',background:'var(--cream)',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',borderRadius:14,marginBottom:14}}>{item.photoUrl?<img src={item.photoUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:44,opacity:.25}}>{CAT_EMOJI[item.category]||'🛍️'}</span>}</div>
...
// ── Outfit collage (mini, for grid card) ──────────────────────────────────────
function OutfitCollage({items}){
  const displayed=items.slice(0,4);
  if(displayed.length===0)return <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:32,opacity:.2}}>✦</div>;
  if(displayed.length===1)return <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',padding:8}}>{displayed[0].photoUrl?<img src={displayed[0].photoUrl} alt="" style={{maxWidth:'100%',maxHeight:'100%',objectFit:'contain',borderRadius:8}}/>:<span style={{fontSize:36,opacity:.4}}>{CAT_EMOJI[displayed[0].category]||'👗'}</span>}</div>;
  return <div className="outfit-collage-inner">
    {displayed.map((item,i)=>(
      <div key={item.id} className={`collage-cell${displayed.length===3&&i===0?' span2':''}`}>
        {item.photoUrl?<img src={item.photoUrl} alt={item.name}/>:<span style={{fontSize:20,opacity:.35}}>{CAT_EMOJI[item.category]||'👗'}</span>}
      </div>
    ))}
  </div>;
}
...
function FreeformCanvas({items,positions,onPositionsChange}){
  ...
  const [pos,setPos]=useState(()=>{
    ...
    items.forEach((item,i)=>{
      p[item.id]=positions?.[item.id]||{x:20+(i%3)*100,y:20+Math.floor(i/3)*130};
    });
    return p;
  });
  ...
  function onMove(e){
    ...
    const newX=Math.max(0,Math.min(canvas.clientWidth-90,x-dragging.current.ox));
    const newY=Math.max(0,Math.min(canvas.clientHeight-110,y-dragging.current.oy));
    setPos(p=>({...p,[dragging.current.id]:{x:newX,y:newY}}));
  }
  ...
  return <div ref={canvasRef} className="freeform-canvas"
    ...
    {items.map(item=>(
      <div key={item.id} className="freeform-item"
        style={{left:pos[item.id]?.x||0,top:pos[item.id]?.y||0}}
        onMouseDown={e=>onStart(e,item.id)} onTouchStart={e=>onStart(e,item.id)}>
        {item.photoUrl?<img src={item.photoUrl} alt={item.name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:28,opacity:.35}}>{CAT_EMOJI[item.category]||'👗'}</span>}
...
function ItemPickerSheet({wardrobe,currentId,slotLabel,onPick,onClose}){
  ...
  const filtered=wardrobe.filter(i=>{
    ...
    const ms=!q||[i.name,i.brand,i.color].some(s=>(s||'').toLowerCase().includes(q.toLowerCase()));
    return mc&&ms;
  });
  return <Sheet title={`Pick ${slotLabel}`} onClose={onClose}>
    <div style={{marginBottom:10}}>
      <input className="f-inp" placeholder="Search…" value={q} onChange={e=>setQ(e.target.value)}/>
    </div>
    <div className="filterrow" style={{padding:'0 0 8px',marginBottom:4}}>
      {CATS.map(c=><button key={c} className={`chip${cat===c?' active':''}`} onClick={()=>setCat(c)} style={{fontSize:11,padding:'4px 10px'}}>{c}</button>)}
    </div>
    <div className="picker-grid">
      {filtered.map(item=>(
        <div key={item.id} className={`picker-item${currentId===item.id?' sel':''}`} onClick={()=>onPick(item)}>
          <div className="picker-thumb">{item.photoUrl?<img src={item.photoUrl} alt={item.name}/>:<span style={{fontSize:22,opacity:.3}}>{CAT_EMOJI[item.category]||'👗'}</span>}</div>
          <div className="picker-name">{item.name}</div>
        </div>
      ))}
      {filtered.length===0&&<div style={{gridColumn:'1/-1',textAlign:'center',padding:'24px 0',fontSize:12,color:'var(--muted)'}}>No items found</div>}
    </div>
  </Sheet>;
}
...
function OutfitBuilderSheet({wardrobe,outfit,onSave,onClose}){
  ...
  const [occasion,setOcc]=useState(outfit?.occasion||'');
  const [notes,setNotes]=useState(outfit?.notes||'');
  const [slots,setSlots]=useState(()=>{
    // slots: {Top,Bottom,Shoes,Bag,Outer} each holds a wardrobe item or null
    ...
    if(outfit?.itemIds){
      outfit.itemIds.forEach(id=>{
        ...
        if(cat==='Tops'||cat==='Dresses'||cat==='Activewear')s.Top=item;
        else if(cat==='Bottoms')s.Bottom=item;
        else if(cat==='Shoes')s.Shoes=item;
        else if(cat==='Bags')s.Bag=item;
        else if(cat==='Outerwear')s.Outer=item;
        else s.Top=s.Top||item;
      });
    }
    return s;
  });
  ...
  const [picker,setPicker]=useState(null); // {slot,label}
  const [saving,setSaving]=useState(false);

  const slotItems=Object.values(slots).filter(Boolean);
  const itemIds=slotItems.map(i=>i.id);

  function setSlotItem(slotKey,item){
    setSlots(p=>({...p,[slotKey]:item}));
    setPicker(null);
  }
  ...
  async function save(){
    setSaving(true);
    await onSave({
      id:outfit?.id||uid(), name:name||'My outfit', occasion, notes,
      itemIds, positions:mode==='freeform'?positions:null,
      wearCount:outfit?.wearCount||0, lastWorn:outfit?.lastWorn??null
    });
    setSaving(false);
  }

  const SLOT_CONFIG=[{key:'Top',label:'Top / Dress'},{key:'Outer',label:'Outer layer'},{key:'Bottom',label:'Bottom'},{key:'Shoes',label:'Shoes'},{key:'Bag',label:'Bag / Acc.'}];

  return <Sheet title={isNew?'New outfit':'Edit outfit'} onClose={onClose}
    actions={<><BtnO onClick={onClose}>Cancel</BtnO><BtnF onClick={save} loading={saving} color="#B07848">{isNew?'Save outfit':'Save changes'}</BtnF></>}>

    <FGrp label="Outfit name"><FInp value={name} placeholder="e.g. Casual Friday" onChange={e=>setName(e.target.value)}/></FGrp>
    <FRow>
      <FGrp label="Occasion" style={{flex:1}}>
        <FSel value={occasion} onChange={e=>setOcc(e.target.value)} placeholder="Any occasion" options={OCCASIONS}/>
      </FGrp>
    </FRow>
    ...
    {mode==='structured'?(
      <div className="builder-stage">
        <div className="structured-layout">
          {SLOT_CONFIG.map(({key,label})=>{
            ...
            return <div key={key} className="slot-row">
              <span className="slot-label">{label}</span>
              {item?(
                <div className="slot-item">
                  <div className="slot-thumb">{item.photoUrl?<img src={item.photoUrl} alt={item.name}/>:<span style={{fontSize:18,opacity:.4}}>{CAT_EMOJI[item.category]||'👗'}</span>}</div>
                  <span className="slot-name" onClick={()=>setPicker({slot:key,label})}>{item.name}</span>
                  <button className="slot-remove" onClick={()=>setSlots(p=>({...p,[key]:null}))}>✕</button>
                </div>
              ):(
                <div className="slot-item empty" onClick={()=>setPicker({slot:key,label})}>
      ...
      <div className="builder-stage freeform">
        <div style={{fontSize:11,color:'var(--muted)',marginBottom:8,textAlign:'center'}}>Drag items to arrange your flat-lay</div>
        {slotItems.length===0?(
          <div style={{textAlign:'center',padding:'32px 0',color:'var(--muted)',fontSize:12}}>Add items in Structured mode first, then drag to arrange</div>
        ):(
          <FreeformCanvas items={slotItems} positions={positions} onPositionsChange={setPositions}/>
        )}
        {slotItems.length>0&&slotItems.length<5&&(
          <div style={{fontSize:11,color:'var(--muted)',textAlign:'center',marginTop:8}}>Switch to Structured to add more items</div>
        )}
      </div>
    )}

    <FGrp label="Notes" style={{marginTop:4}}><FInp value={notes} placeholder="What you love about this combo, when you'd wear it…" onChange={e=>setNotes(e.target.value)}/></FGrp>

    {picker&&<ItemPickerSheet wardrobe={wardrobe} currentId={slots[picker.slot]?.id} slotLabel={picker.label} onPick={item=>setSlotItem(picker.slot,item)} onClose={()=>setPicker(null)}/>}
  </Sheet>;
}
...
function OutfitDetailSheet({outfit,wardrobe,onClose,onEdit,onDelete,onMarkWorn}){
  ...
  return <Sheet title={outfit.name} onClose={onClose}>
    ...
    <div style={{marginBottom:14}}>
      ...
      {freeform&&outfit.positions?(
        <FreeformCanvas items={items} positions={outfit.positions} onPositionsChange={()=>{}}/>
      ):(
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
          {items.map(item=>(
            <div key={item.id} style={{borderRadius:10,overflow:'hidden',background:'var(--cream)',aspectRatio:'3/4',display:'flex',alignItems:'center',justifyContent:'center'}}>
              {item.photoUrl?<img src={item.photoUrl} alt={item.name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:28,opacity:.3}}>{CAT_EMOJI[item.category]||'👗'}</span>}
            </div>
          ))}
        </div>
      )}
    ...
    <DetailRow label="Occasion" value={outfit.occasion}/>
    <DetailRow label="Last worn" value={daysLabel(outfit.lastWorn)}/>
    <DetailRow label="Times worn" value={outfit.wearCount||0}/>
    <DetailRow label="Notes" value={outfit.notes}/>

    <div style={{marginTop:12}}>
      <div style={{fontSize:10,letterSpacing:1,textTransform:'uppercase',color:'var(--muted)',marginBottom:8}}>Items in this outfit</div>
      {items.map(item=>(
        <div key={item.id} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 0',borderBottom:'1px solid var(--border)'}}>
          <div style={{width:32,height:40,borderRadius:6,background:'var(--cream)',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
            {item.photoUrl?<img src={item.photoUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:14,opacity:.3}}>{CAT_EMOJI[item.category]||'👗'}</span>}
          </div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:12,fontWeight:500,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{item.name}</div>
            <div style={{fontSize:10,color:'var(--muted)'}}>{item.store||item.brand||item.category}</div>
          </div>
          <div style={{fontSize:10,color:'var(--muted)',flexShrink:0}}>{daysLabel(item.lastWorn)}</div>
        </div>
      ))}
    </div>

    <div style={{display:'flex',gap:8,marginTop:14}}>
      <button onClick={onMarkWorn} style={{flex:1,padding:12,background:'var(--green)',color:'#fff',border:'none',borderRadius:12,fontSize:13,cursor:'pointer',fontFamily:"'Jost',sans-serif"}}>✓ Wore this today</button>
      <button onClick={onEdit} style={{flex:1,padding:12,border:'1.5px solid var(--border)',borderRadius:12,background:'none',fontSize:13,cursor:'pointer',fontFamily:"'Jost',sans-serif"}}>Edit</button>
      <button onClick={onDelete} style={{padding:'12px 14px',border:'1.5px solid #EAC8C8',borderRadius:12,background:'none',fontSize:13,cursor:'pointer',color:'var(--red)',fontFamily:"'Jost',sans-serif"}}>🗑</button>
    </div>
  </Sheet>;
...
function WardrobeCard({item,onClick}){
  return <div className={`icard${!item.complete?' incomplete':''}`} onClick={onClick}>
    <div className="iphoto">
      ...
      {item.lastWorn!==null&&item.lastWorn!==undefined&&<span className="badge-worn">{daysLabel(item.lastWorn)}</span>}
    </div>
    <div className="iinfo">
...
function WishCard({item,similar,onRate,onClick}){
  return <div className="wcard" onClick={onClick}>
    <div className="wimg">{item.photoUrl?<img src={item.photoUrl} alt={item.name}/>:<span style={{fontSize:32,opacity:.25}}>{CAT_EMOJI[item.category]||'🛍️'}</span>}</div>
    <div style={{padding:'10px 12px'}}>
      <div style={{fontSize:12,fontWeight:500,color:'var(--ink)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',marginBottom:3}}>{item.name}</div>
      <div style={{fontSize:10,color:'var(--muted)'}}>{item.store||item.brand||'—'}</div>
      <div style={{display:'flex',alignItems:'center',gap:6,marginTop:4}}>
        {item.price&&<span style={{fontSize:13,fontWeight:500,color:item.onSale?'var(--red)':'var(--ink)'}}>A${item.price}</span>}
        {item.onSale&&item.origPrice&&<span style={{fontSize:11,color:'var(--muted)',textDecoration:'line-through'}}>A${item.origPrice}</span>}
        {item.onSale&&<span style={{fontSize:9,background:'#FEF0F0',color:'var(--red)',borderRadius:4,padding:'2px 5px',fontWeight:500}}>Sale</span>}
      </div>
      <div onClick={e=>e.stopPropagation()} style={{marginTop:4}}><StarDisplay rating={item.rating||3} size={13} onClick={onRate}/></div>
...
function App(){
  ...
  // Store
  async function addStore(name){const s={id:uid(),name,country:'AU'};try{await sb.ins('stores',s);setStores(p=>[...p,s]);}catch(e){console.error(e);}}

  // Wardrobe CRUD
  async function addItem(item){try{await sb.ins('wardrobe',toRow(item));setW(p=>[...p,item]);}catch(e){console.error(e);}setShowAddW(false);}
  async function saveItem(item){const complete=!!(item.name&&item.category&&(item.size||item.brand));const u={...item,complete};try{await sb.upd('wardrobe',item.id,toRow(u));setW(p=>p.map(x=>x.id===item.id?u:x));setSelItem(u);}catch(e){console.error(e);}setEditItem(false);}
  async function delItem(id){try{await sb.del('wardrobe',id);setW(p=>p.filter(x=>x.id!==id));}catch(e){console.error(e);}setSelItem(null);}
  async function markWorn(item){const u={...item,lastWorn:0,wearCount:(item.wearCount||0)+1};try{await sb.upd('wardrobe',item.id,{last_worn:0,wear_count:u.wearCount});setW(p=>p.map(x=>x.id===item.id?u:x));setSelItem(u);}catch(e){console.error(e);}}

  // Wishlist CRUD
  async function addWish(item){try{await sb.ins('wishlist',toWR(item));setWL(p=>[...p,item]);}catch(e){console.error(e);}setShowAddWL(false);}
  async function saveWish(item){try{await sb.upd('wishlist',item.id,toWR(item));setWL(p=>p.map(x=>x.id===item.id?item:x));setSelWish(item);}catch(e){console.error(e);}setEditWish(false);}
  ...
  async function bought(w){const item={...w,id:uid(),lastWorn:null,wearCount:0,complete:false,dateBought:today(),notes:`From wishlist. ${w.notes||''}`.trim()};await addItem(item);await delWish(w.id);}

  // Outfit CRUD
  async function saveOutfit(outfit){
    const row=toOR(outfit);
    try{
      if(outfits.find(o=>o.id===outfit.id)){await sb.upd('outfits',outfit.id,row);setO(p=>p.map(x=>x.id===outfit.id?outfit:x));setSelOutfit(outfit);}
      else{await sb.ins('outfits',row);setO(p=>[...p,outfit]);}
    }catch(e){console.error(e);}
  ...
  async function markOutfitWorn(outfit){
    ...
    const u={...outfit,lastWorn:0,wearCount:(outfit.wearCount||0)+1};
    try{await sb.upd('outfits',outfit.id,{last_worn:0,wear_count:u.wearCount});setO(p=>p.map(x=>x.id===outfit.id?u:x));setSelOutfit(u);}catch(e){console.error(e);}
    // also increment each item's wear count
    for(const id of outfit.itemIds){
      const item=wardrobe.find(w=>w.id===id);if(!item)continue;
      const ui={...item,lastWorn:0,wearCount:(item.wearCount||0)+1};
      try{await sb.upd('wardrobe',id,{last_worn:0,wear_count:ui.wearCount});setW(p=>p.map(x=>x.id===id?ui:x));}catch(e){console.error(e);}
    }
  }
  ...
  if(loading)return <>
    <style>{CSS}</style>
    <div style={{height:'100dvh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--cream)',flexDirection:'column',gap:12}}>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:32,fontWeight:300,letterSpacing:4}}>closet</div>
      <div style={{fontSize:12,color:'var(--muted)'}}>Loading your wardrobe…</div>
    </div>
  </>;
  ...
  return <>
    ...
    <div className="app"><div className="app-inner">
      ...
      <div className="sidebar">
        ...
        <nav style={{display:'flex',flexDirection:'column',flex:1}}>
          {NAV.map(n=><button key={n.key} className={`sidebar-item${tab===n.key?' active':''}`} onClick={()=>setTab(n.key)}>{n.label}</button>)}
        </nav>
      ...
      <div className="content-area">
        <div className="topbar">
          ...
          {(tab==='wardrobe'||tab==='wishlist')&&<div className="searchbox">
            <span style={{fontSize:13,color:'var(--muted)'}}>⌕</span>
            <input placeholder={tab==='wardrobe'?'Search items, brands, colours…':'Search wishlist…'} value={search} onChange={e=>setSearch(e.target.value)}/>
            {search&&<span onClick={()=>setSearch('')} style={{cursor:'pointer',color:'var(--muted)',fontSize:12}}>✕</span>}
          </div>}
        ...
        <div className="scroll">
          ...
          {/* OUTFITS */}
          {tab==='outfits'&&<>
            {outfits.length===0?<div className="empty">
              <div style={{fontSize:40,marginBottom:10}}>✦</div>
              <div className="empty-t">No saved outfits yet</div>
              <div className="empty-s">Tap + to build your first outfit. Pick items from your wardrobe, arrange them as a flat-lay, and save the combination.</div>
            </div>:<>
              ...
              <div className="outfit-grid">
                {outfits.map(outfit=>{
                  ...
                  return <div key={outfit.id} className="outfit-card" onClick={()=>{setSelOutfit(outfit);setEditOutfit(false);}}>
                    <div className="outfit-collage"><OutfitCollage items={items}/></div>
                    <div className="outfit-info">
                      <div className="outfit-name">{outfit.name}</div>
                      <div className="outfit-meta">{items.length} pieces{outfit.occasion?` · ${outfit.occasion}`:''}{outfit.wearCount?` · worn ${outfit.wearCount}×`:''}</div>
                    </div>
                  </div>;
                })}
              </div>
          ...
          {tab==='wishlist'&&<>
            ...
            {fWL.length===0?<div className="empty"><div style={{fontSize:40,marginBottom:10}}>🛍️</div><div className="empty-t">Nothing on the wishlist yet</div><div className="empty-s">Tap + to add items. Paste a link and the store name fills in automatically.</div></div>
          ...
          {tab==='stats'&&<>
            <div className="stat-grid">
              {[{n:wardrobe.filter(i=>i.complete).length,l:'Items in wardrobe'},{n:outfits.length,l:'Saved outfits'},{n:wishlist.length,l:'On wishlist'},{n:unworn.length,l:'Unworn 30+ days'}].map(s=><div key={s.l} className="stat-card"><div className="stat-n">{s.n}</div><div className="stat-l">{s.l}</div></div>)}
            </div>
          ...
          {tab==='ask'&&<div className="ask-wrap">
            <div className="ask-box">
              ...
              <div>{ASK_SUGG.map(q=><div key={q} onClick={()=>handleAsk(q)} style={{padding:'9px 14px',fontSize:12,color:'var(--muted)',cursor:'pointer',borderTop:'1px solid var(--border)'}}>{q}</div>)}</div>
            </div>
            {askResult&&<div style={{background:'var(--ink)',borderRadius:14,padding:16,color:'#fff'}}>
              ...
              {askResult.notFound?<><div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:300,marginBottom:6}}>Nothing found</div><div style={{fontSize:12,color:'rgba(255,255,255,.7)',lineHeight:1.7}}>No items matched. Try a brand, colour, or category.</div></>
              :<><div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:300,marginBottom:6,lineHeight:1.3}}>{askResult.item.name} — {askResult.item.sizeLabel||askResult.item.size||'size not set'}</div><div style={{fontSize:12,color:'rgba(255,255,255,.7)',lineHeight:1.7,whiteSpace:'pre-line'}}>{[askResult.item.brand,askResult.item.color,askResult.item.category,askResult.item.subcategory,askResult.item.store&&`from ${askResult.item.store}`].filter(Boolean).join(' · ')}{askResult.sRow?`\n\n${askResult.guide.store} size ${askResult.sRow.l}:\nBust ${askResult.sRow.bust}cm · Waist ${askResult.sRow.waist}cm · Hip ${askResult.sRow.hip}cm${askResult.sRow.inseam?` · Inseam ${askResult.sRow.inseam}`:''}`:askResult.guide?'\n\nSize guide available — check the Sizes tab.':''}</div></>}
            </div>}
        ...
        <div className="botnav">
          {NAV.map(n=><button key={n.key} className={`navitem${tab===n.key?' active':''}`} onClick={()=>setTab(n.key)}>
            <span className="nl">{n.label}</span>
            {tab===n.key&&<span className="navdot"/>}
          </button>)}
        </div>
      </div>{/* content-area */}
    </div></div>{/* app-inner / app */}
    ...
    {selOutfit&&!editOutfit && <OutfitDetailSheet outfit={selOutfit} wardrobe={wardrobe} onClose={()=>setSelOutfit(null)} onEdit={()=>setEditOutfit(true)} onDelete={()=>delOutfit(selOutfit.id)} onMarkWorn={()=>markOutfitWorn(selOutfit)}/>}
