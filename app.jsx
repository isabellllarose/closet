...
function PhotoZone({photoUrl,onFile,style={},uploading=false}){const ref=useRef();return <div className="pz" style={style} onClick={()=>ref.current?.click()}>{photoUrl&&<img src={photoUrl} alt=""/>}{!photoUrl&&!uploading&&<><span style={{fontSize:26,zIndex:1}}>📷</span><span style={{fontSize:11,color:'var(--muted)',zIndex:1,textAlign:'center',lineHeight:1.4}}>Tap to add photo{'\n'}camera or library</span></>}{uploading&&<div style={{zIndex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:6}}><div className="spinner" style={{borderTopColor:'var(--accent)',borderColor:'var(--border)'}}/><span style={{fontSize:11,color:'var(--muted)'}}>Uploading…</span></div>}<input ref={ref} type="file" accept="image/*" onChange={e=>{const f=e.target.files?.[0];if(f)onFile(f);}}/></div>;}
...
function WardrobeFields({d,up,stores,onAddStore}){
  ...
  return <>
    ...
    <FRow>
      <FGrp label="Brand" style={{flex:1}}><FInp value={d.brand} placeholder="e.g. Levi's" onChange={e=>up({brand:e.target.value})}/></FGrp>
      <FGrp label="Colour" style={{flex:1}}><FInp value={d.color} placeholder="Navy" onChange={e=>up({color:e.target.value})}/></FGrp>
    </FRow>
    <FRow>
      <FGrp label="Size" style={{flex:1}}><FInp value={d.size} placeholder="S / 10 / 27" onChange={e=>up({size:e.target.value,sizeLabel:e.target.value})}/></FGrp>
      <FGrp label="Store" style={{flex:1}}><StorePicker value={d.store||''} onChange={v=>up({store:v})} stores={stores} onAddStore={onAddStore}/></FGrp>
    </FRow>
...
function WardrobeDetailSheet({item,onClose,onEdit,onLogWear,onDelete}){
  ...
  return <Sheet title={item.name||'Untitled'} onClose={onClose}>
    ...
    <DetailRow label="Last worn" value={item.lastWornDate?`${formatDate(item.lastWornDate)} (${daysAgoLabel(item.lastWornDate)})`:'Never worn'}/>
    <DetailRow label="Total wears" value={item.wearCount||0}/>
    <DetailRow label="Occasions" value={(item.occasions||[]).join(', ')||null}/>
  ...
  return <Sheet title={isNew?'New outfit':'Edit outfit'} onClose={onClose} actions={<><BtnO onClick={onClose}>Cancel</BtnO><BtnF onClick={save} loading={saving} color="#B07848">{isNew?'Save outfit':'Save changes'}</BtnF></>}>
    ...
    {mode==='structured'?(
      <div className="builder-stage">
        {SLOT_CONFIG.map(({key,label})=>{const item=slots[key];return <div key={key} className="slot-row">
      ...
      <div className="builder-stage">
        ...
        {slotItems.length===0?<div style={{textAlign:'center',padding:'32px 0',color:'var(--muted)',fontSize:12}}>Add items in Structured mode first, then drag to arrange</div>
  ...
  return <Sheet title={outfit.name} onClose={onClose}>
    ...
    <div style={{marginTop:12}}>
      ...
      {items.map(item=><div key={item.id} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 0',borderBottom:'1px solid var(--border)'}}>
        <div style={{width:32,height:40,borderRadius:6,background:'var(--cream)',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{item.photoUrl?<img src={item.photoUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:14,opacity:.3}}>{CAT_EMOJI[item.category]||'👗'}</span>}</div>
...
function WishCard({item,similar,onRate,onClick}){
  return <div className="wcard" onClick={onClick}>
    ...
    <div style={{padding:'10px 12px'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:6,marginTop:4}}>{item.price&&<span style={{fontSize:13,fontWeight:500,color:item.onSale?'var(--red)':'var(--ink)'}}>A${item.price}</span>}{item.onSale&&item.origPrice&&<span style={{fontSize:11,color:'var(--muted)',textDecoration:'line-through'}}>A${item.origPrice}</span>}{item.onSale&&<span style={{fontSize:9,background:'#FEF0F0',color:'var(--red)',borderRadius:4,padding:'2px 5px',fontWeight:500}}>Sale</span>}</div>
